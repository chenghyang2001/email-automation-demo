...
**版本基準**：初版（2026-06-18，commit ee0a479）
**取樣範圍**：全專案 8 個 `.py`（共 1412 行）逐檔查證，含 `.github/workflows/` 兩個 CI/CD 設定。
...
```
            ...
            ▼                      ▼         ▼                         ▼
      ┌──────────┐         ┌────────────┐ ┌──────────────┐    ┌────────────────┐
      │ cache.py │         │email_filter│ │ summarizer.py│    │telegram_briefer│
      │ 冪等去重 │         │ L1規則+L2AI│ │ AI→JSON摘要  │    │  格式化+發送   │
      └──────────┘         └────────────┘ └──────────────┘    └────────────────┘
                                  │              │                     ▲
                                  │              ▼                     │
                                  │       ┌──────────────┐            │
...
每天早上掃描 Gmail，把「需要行動」的信件經 AI 兩層過濾與摘要，轉成 Notion 任務卡片，
並透過 Telegram 發出晨間簡報。核心價值不在演算法，而在**一份程式碼如何同時適配「本機 Max 訂閱」
...
| 層級 | 檔案 | 行數 | 角色 |
|------|------|:---:|------|
| **進入點** | `run_briefing.py` | 228 | 主協調器（Orchestrator）。解析 `--dry-run`、環境檢查、依序串接五模組、Graceful Failure 包裹每封信、最終統計輸出 |
...
| **核心模組** | `notion_creator.py` | 220 | 建立 Notion 任務卡片。`priority_score`→P1/P2/P3 映射（雙通路：notion-client SDK / claude -p + MCP）|
| **核心模組** | `telegram_briefer.py` | 167 | 純函式格式化晨報字串 + Telegram Bot API 發送（失敗重試一次）|
...
Notion（MCP 或 notion-client SDK）、Telegram Bot API（直接 HTTP POST）。
...
all_emails ──► [cache 去重] ──► new_emails ──► [filter_emails] ──► filtered
   ...
   summarize_email ──► create_notion_task ──► mark_processed（每封即時更新 cache）
            │
            ▼ 全部處理完
   format_briefing ──► send_telegram ──► 最終統計
```
...
7. format_briefing 組裝 → send_telegram（HTTP POST，重試 1 次）
```
...
| ADR-7 | 截止日在 summarizer 標準化為 ISO | 下游 Notion/Telegram 需可排序日期，不能用「Thursday EOD」 | 自然語言解析涵蓋有限（僅星期/today/tomorrow/EOD/ISO）|
...
打包（git push main）
  └─ ci.yml 觸發
         ├─ py_compile 7 檔語法檢查
         ├─ Smoke tests：email_filter / summarizer / notion_creator 各自 __main__
         └─ Dry run：run_briefing.py --dry-run（dummy env）
         │
發佈（GitHub Actions 排程）
  └─ daily-briefing.yml
         ├─ cron 30 23 * * 0-4（UTC）= 台灣 07:30 週一~五
         ├─ Secrets：GMAIL_TOKEN_JSON / ANTHROPIC_API_KEY / NOTION_API_KEY
...
端到端乾跑（`--dry-run`）→ 真實整合（手動確認 Telegram/Notion 產出）。
